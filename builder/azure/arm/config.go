...
const (
	...
	keyVaultSecretNameMaxLength       = 127
	keyVaultSecretNameSuffixLength    = 16
	keyVaultSecretNamePrefixMaxLength = keyVaultSecretNameMaxLength - keyVaultSecretNameSuffixLength - 1
	keyVaultSecretNameSuffixAlphabet  = "0123456789abcdefghijklmnopqrstuvwxyz"
)
...
var (
	...
	reKeyVaultSecretPrefix = regexp.MustCompile(fmt.Sprintf(`^[0-9A-Za-z-]{1,%d}$`, keyVaultSecretNamePrefixMaxLength))
)
...
type Config struct {
	...
	// run-scoped secret name generated by Packer. The prefix must contain only
	// alphanumeric characters or hyphens and be at most 110 characters.
	BuildKeyVaultSecretName string `mapstructure:"build_key_vault_secret_name"`
	// Delete the Packer-created certificate secret from an existing build Key Vault during cleanup.
	// This requires build_key_vault_name and build_resource_group_name for the existing
	// Key Vault in the configured subscription, plus Azure Resource Manager read permission
	// on that Key Vault and Key Vault data-plane secrets/delete permission.
	// This is supported only for Windows builds and cannot be used with skip_create_build_key_vault.
	// Packer appends a random suffix to build_key_vault_secret_name so each build uses
	// a dedicated secret. Deleting a secret removes all its versions and normally soft-deletes it.
	// Key Vault can complete deletion asynchronously; Packer does not purge it. The
...
func (c *Config) Prepare(raws ...interface{}) ([]string, error) {
	...
	if c.BuildKeyVaultDeleteSecret {
		keyVaultSecretName, err := generateKeyVaultSecretName(c.BuildKeyVaultSecretName)
		if err != nil {
			return nil, err
		}
		c.tmpKeyVaultSecretName = keyVaultSecretName
	}
...
func generateKeyVaultSecretName(prefix string) (string, error) {
	suffix := make([]byte, keyVaultSecretNameSuffixLength)
	for i := range suffix {
		index, err := rand.Int(rand.Reader, big.NewInt(int64(len(keyVaultSecretNameSuffixAlphabet))))
		if err != nil {
			return "", fmt.Errorf("generating Key Vault secret name: %w", err)
		}
		suffix[i] = keyVaultSecretNameSuffixAlphabet[index.Int64()]
	}
...
func assertRequiredParametersSet(c *Config, errs *packersdk.MultiError) {
	...
	if c.BuildKeyVaultDeleteSecret && c.BuildKeyVaultName == "" {
		errs = packersdk.MultiErrorAppend(errs, fmt.Errorf("build_key_vault_delete_secret requires build_key_vault_name"))
	}
	if c.BuildKeyVaultDeleteSecret && c.BuildResourceGroupName == "" {
		errs = packersdk.MultiErrorAppend(errs, fmt.Errorf("build_key_vault_delete_secret requires build_resource_group_name for the Key Vault resource group"))
	}
	if c.BuildKeyVaultDeleteSecret && c.SkipCreateBuildKeyVault {
		errs = packersdk.MultiErrorAppend(errs, fmt.Errorf("build_key_vault_delete_secret cannot be used with skip_create_build_key_vault"))
	}
	if c.BuildKeyVaultDeleteSecret && !strings.EqualFold(c.OSType, constants.Target_Windows) {
		errs = packersdk.MultiErrorAppend(errs, fmt.Errorf("build_key_vault_delete_secret is only supported for Windows builds"))
	}
