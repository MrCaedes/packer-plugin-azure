...
- `build_key_vault_delete_secret` (bool) - Delete the Packer-created certificate secret from an existing build Key Vault during cleanup.
...
- `build_key_vault_enable_rbac_authorization` (bool) - Enable Azure RBAC authorization for the temporary build Key Vault instead
...
#### WinRM NTLM (Azure-specific default)

~> **Note:** Unlike the generic SDK default, this plugin enables NTLM authentication
for WinRM automatically when `winrm_use_ntlm` is not set. Azure Windows VMs use
Negotiate/NTLM auth by default, and Basic auth is typically rejected.

- **unset** (default) — NTLM is used. No action required for standard Azure Windows images.
- **`true`** — NTLM is used (same as unset).
- **`false`** — NTLM is disabled. Use this for STIG/FIPS-hardened images that enforce
  Extended Protection for Authentication (EPA) with Channel Binding Tokens, which
  are incompatible with the NTLM client.
